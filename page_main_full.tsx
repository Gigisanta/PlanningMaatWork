  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
      <div className="p-4 border-b border-[#E8E6E0] flex items-center justify-between bg-[#F5F4F0]">
        ...
        <Button variant="ghost" size="icon" onClick={onClose} aria-label="Cerrar configuración"><X className="w-6 h-6" /></Button>
      </div>
      ...
      <header className="bg-[#2D5A4A] text-white md:py-3 py-2.5 px-4 shadow-sm flex-shrink-0 sticky top-0 z-50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
                                    ...
                                    {portfolioLibrary.length === 0 ? <p className="text-center py-8 text-[#7A8B80] text-xs">No hay carteras guardadas.</p> :
                                        portfolioLibrary.map(item => (
                                            <div key={item.id} className="p-3 bg-white rounded-xl flex items-center justify-between border border-[#E8E6E0] hover:border-[#C4846C] transition-colors group">
                                                ...
                                                <Button variant="ghost" size="sm" onClick={() => handleDeleteFromLibrary(item.id)} aria-label="Eliminar cartera guardada" className="text-red-400 opacity-0 group-hover:opacity-100 h-8 w-8 p-0"><Trash2 className="w-3.5 h-3.5"/></Button>
                                            </div>
                                        ))
                                    }
                                </div>
            ...
            {isMobile ? (
              <Button variant="outline" size="icon" onClick={() => setShowMobileSettings(true)} aria-label="Abrir configuración" className="bg-white/10 border-white/15 text-white hover:bg-white/20 rounded-lg h-10 w-10"><Settings className="w-5 h-5" /></Button>
            ) : (
              <Popover>
