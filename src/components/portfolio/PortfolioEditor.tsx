                ...
                <div className="mt-3 space-y-2">
                  {attachedFiles.map((file, i) => (
                    <div key={i} className={`flex items-center justify-between ${isMobile ? 'p-3' : 'p-2'} bg-[#F5F4F0] rounded-xl ${isMobile ? '' : 'rounded'} text-sm group`}>
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setAttachedFiles((prev: any) => prev.filter((_: any, j: number) => j !== i))} aria-label="Eliminar archivo adjunto" className={`${isMobile ? 'h-10 w-10' : 'h-5 w-5 p-0 opacity-0 group-hover:opacity-100'}`}><X className="w-4 h-4" /></Button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-3">
              {metasVida.map((meta: any, i: number) => (
                <div key={meta.id} className="p-4 bg-white rounded-2xl border border-[#E8E6E0] shadow-sm relative group transition-all hover:border-[#3D7A5F]/30">
                  <Button variant="ghost" size="sm" onClick={() => setMetasVida(metasVida.filter((m: any) => m.id !== meta.id))} aria-label="Eliminar meta" className="absolute top-2 right-2 h-7 w-7 text-red-300 opacity-0 group-hover:opacity-100"><X className="w-4 h-4" /></Button>
                  ...
                  <div key={i} className={`p-4 bg-white rounded-2xl group relative border ${totalAsignacion !== 100 && !inst.locked ? 'border-amber-200' : 'border-[#E8E6E0]'} shadow-sm hover:shadow-md transition-shadow`}>
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <Button variant="ghost" size="sm" onClick={() => { const arr = [...instruments]; arr[i] = { ...arr[i], locked: !arr[i].locked }; setInstruments(arr); }} aria-label={inst.locked ? "Desbloquear asignación" : "Bloquear asignación"} className={`h-9 w-9 rounded-full ${inst.locked ? 'bg-amber-50 text-[#C4846C]' : 'bg-[#F5F4F0] text-[#7A8B80]'}`}>{inst.locked ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}</Button>
                        ...
                        <div className="flex items-center gap-2">
                             ...
                             <Button variant="ghost" size="sm" onClick={() => setInstruments((prev: any) => prev.filter((_: any, j: number) => j !== i))} aria-label="Eliminar instrumento" className="h-9 w-9 rounded-full text-red-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="w-4.5 h-4.5" /></Button>
                        </div>
                    </div>
              ...
              <div className="space-y-2">
                {asignacionEstrategica.map((asig, i) => (
                  <div key={i} className={`flex items-center gap-3 p-3 bg-white rounded-2xl border ${totalAsignacionEstrategica !== 100 ? 'border-amber-100' : 'border-[#E8E6E0]'} shadow-sm transition-all`}>
                    <Button variant="ghost" size="sm" onClick={() => { const arr = [...asignacionEstrategica]; arr[i] = { ...arr[i], locked: !arr[i].locked }; setAsignacionEstrategica(arr); }} aria-label={asig.locked ? "Desbloquear asignación estratégica" : "Bloquear asignación estratégica"} className={`h-8 w-8 rounded-full ${asig.locked ? 'bg-amber-50 text-[#C4846C]' : 'bg-[#F5F4F0] text-[#7A8B80]'}`}>{asig.locked ? <Lock className="w-3.5 h-3.5" /> : <Unlock className="w-3.5 h-3.5" />}</Button>
            ...
            <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {obligacionesNegociables.map((on, i) => (
                      <div key={i} className="p-4 bg-white rounded-2xl border border-[#E8E6E0] shadow-sm relative group">
                        <Button variant="ghost" size="sm" onClick={() => setObligacionesNegociables((prev: any) => prev.filter((_: any, j: number) => j !== i))} aria-label="Eliminar obligación negociable" className="absolute top-2 right-2 h-7 w-7 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4"/></Button>
                        <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {riesgos.map((r, i) => (
                      <div key={i} className="p-4 bg-white rounded-2xl border border-[#E8E6E0] shadow-sm relative group">
                        <Button variant="ghost" size="sm" onClick={() => setRiesgos((prev: any) => prev.filter((_: any, j: number) => j !== i))} aria-label="Eliminar riesgo" className="absolute top-2 right-2 h-7 w-7 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4"/></Button>
                  ...
                  <div className="space-y-2">
                    {beneficiosFiscales.map((b, i) => (
                      <div key={i} className="flex gap-2 group">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => setBeneficiosFiscales((prev: string[]) => prev.filter((_, j) => j !== i))} aria-label="Eliminar beneficio fiscal" className="h-auto px-2 text-red-300 hover:text-red-600 transition-colors"><Trash2 className="w-4 h-4"/></Button>
                      </div>
                    ))}
                  </div>
                </div>
