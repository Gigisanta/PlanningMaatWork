                ...
                <div className="mt-3 space-y-2">
                  {attachedFiles.map((file, i) => (
                    <div key={i} className={`flex items-center justify-between ${isMobile ? 'p-3' : 'p-2'} bg-[#F5F4F0] rounded-xl ${isMobile ? '' : 'rounded'} text-sm group`}>
                      ...
                      <Button variant="ghost" size="sm" aria-label="Eliminar documento PDF" onClick={() => setAttachedFiles((prev: any) => prev.filter((_: any, j: number) => j !== i))} className={`${isMobile ? 'h-10 w-10' : 'h-5 w-5 p-0 opacity-0 group-hover:opacity-100'}`}><X className="w-4 h-4" /></Button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {obligacionesNegociables.map((on, i) => (
                      <div key={i} className="p-4 bg-white rounded-2xl border border-[#E8E6E0] shadow-sm relative group">
                        <Button variant="ghost" size="sm" aria-label="Eliminar obligación negociable" onClick={() => setObligacionesNegociables((prev: any) => prev.filter((_: any, j: number) => j !== i))} className="absolute top-2 right-2 h-7 w-7 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4"/></Button>
                        <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid grid-cols-1 gap-4 pt-4">
                  <div className="p-4 bg-white rounded-3xl border-2 border-dashed border-[#E8E6E0]">
                    <div className="flex items-center gap-2 mb-3"><input id="checkbox-termino-ia" type="checkbox" checked={usarTerminoIA} onChange={(e) => setUsarTerminoIA(e.target.checked)} className="w-5 h-5 rounded-lg border-[#E8E6E0] text-[var(--primary-light)] focus:ring-[#3D7A5F] cursor-pointer" /><Label htmlFor="checkbox-termino-ia" className="text-xs font-black uppercase text-[#1F2D26] cursor-pointer">Término IA Autogenerado</Label></div>
                  ...
                  <div className="p-4 bg-white rounded-3xl border-2 border-dashed border-[#E8E6E0]">
                    <div className="flex items-center gap-2 mb-3"><input id="checkbox-consejo-ia" type="checkbox" checked={usarConsejoIA} onChange={(e) => setUsarConsejoIA(e.target.checked)} className="w-5 h-5 rounded-lg border-[#E8E6E0] text-[var(--primary-light)] focus:ring-[#3D7A5F] cursor-pointer" /><Label htmlFor="checkbox-consejo-ia" className="text-xs font-black uppercase text-[#1F2D26] cursor-pointer">Consejo IA Autogenerado</Label></div>
