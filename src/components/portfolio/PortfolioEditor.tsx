                ...
                <div className="mt-3 space-y-2">
                  {attachedFiles.map((file, i) => (
                    <div key={i} className={`flex items-center justify-between ${isMobile ? 'p-3' : 'p-2'} bg-[#F5F4F0] rounded-xl ${isMobile ? '' : 'rounded'} text-sm group`}>
                      ...
                      <Button variant="ghost" size="sm" aria-label="Eliminar documento" onClick={() => setAttachedFiles((prev: any) => prev.filter((_: any, j: number) => j !== i))} className={`${isMobile ? 'h-10 w-10' : 'h-5 w-5 p-0 opacity-0 group-hover:opacity-100'}`}><X className="w-4 h-4" /></Button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-3">
              {metasVida.map((meta: any, i: number) => (
                <div key={meta.id} className="p-4 bg-white rounded-2xl border border-[#E8E6E0] shadow-sm relative group transition-all hover:border-[#3D7A5F]/30">
                  <Button variant="ghost" size="sm" aria-label="Eliminar meta" onClick={() => setMetasVida(metasVida.filter((m: any) => m.id !== meta.id))} className="absolute top-2 right-2 h-7 w-7 text-red-300 opacity-0 group-hover:opacity-100"><X className="w-4 h-4" /></Button>
