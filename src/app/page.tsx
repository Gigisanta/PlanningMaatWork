    ...
    <div className="min-h-screen bg-[#FAFAF8] flex flex-col" style={{"--primary": colorPrincipal || "#2D5A4A", "--primary-light": colorPrincipal ? `color-mix(in srgb, ${colorPrincipal} 80%, white)` : "#3D7A5F", "--primary-dark": colorPrincipal ? `color-mix(in srgb, ${colorPrincipal} 80%, black)` : "#1F4A3D", "--accent": colorAcento || "#C4846C", "--accent-light": colorAcento ? `color-mix(in srgb, ${colorAcento} 80%, white)` : "#D9A58B", "--accent-dark": colorAcento ? `color-mix(in srgb, ${colorAcento} 80%, black)` : "#A36D59"} as React.CSSProperties}>
      ...
      <div className="flex-1 flex overflow-hidden">
        {!isMobile ? (
          ...
          <>
            <div id="panel-form" role="tabpanel" aria-labelledby="tab-form" aria-hidden={mobilePanel !== 'form'} className={`fixed inset-0 top-[60px] z-40 bg-white flex flex-col transition-transform duration-300 ${mobilePanel === 'form' ? 'translate-x-0' : '-translate-x-full'}`} style={{ bottom: 'calc(64px + env(safe-area-inset-bottom, 0px))' }}>
              <PortfolioEditor {...editorProps} />
            ...
            <div id="panel-preview" role="tabpanel" aria-labelledby="tab-preview" aria-hidden={mobilePanel !== 'preview'} className={`fixed inset-0 top-[60px] z-40 bg-[#F5F4F0] flex flex-col transition-transform duration-300 ${mobilePanel === 'preview' ? 'translate-x-0' : 'translate-x-full'}`} style={{ bottom: 'calc(64px + env(safe-area-inset-bottom, 0px))' }}>
              <PortfolioPreview {...previewProps} />
            </div>
          </>
        )}
      ...
      {isMobile && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#E8E6E0] z-50 safe-area-bottom h-[calc(64px+env(safe-area-inset-bottom,0px))]">
          <div className="flex h-16" role="tablist" aria-label="Navegación móvil">
            <button id="tab-form" role="tab" aria-selected={mobilePanel === 'form'} aria-controls="panel-form" onClick={() => setMobilePanel('form')} className={`flex-1 flex flex-col items-center justify-center gap-1 transition-colors relative ${mobilePanel === 'form' ? 'text-[var(--primary)]' : 'text-[#7A8B80]'}`}>
            ...
            <button id="tab-preview" role="tab" aria-selected={mobilePanel === 'preview'} aria-controls="panel-preview" onClick={() => setMobilePanel('preview')} className={`flex-1 flex flex-col items-center justify-center gap-1 transition-colors relative ${mobilePanel === 'preview' ? 'text-[var(--primary)]' : 'text-[#7A8B80]'}`}>
