...
function adjustWeights<T extends { locked?: boolean }>(items: T[], changedIndex: number, newValue: number, key: keyof T): T[] {
  ...
  item[key] = newValue as unknown as T[keyof T]
  ...
  unlockedItems.forEach(ui => { ui[key] = Math.max(0, (ui[key] as number) - diffPerItem) as unknown as T[keyof T] })
...
function normalizeWeights<T extends { locked?: boolean }>(items: T[], key: keyof T): T[] {
  ...
  newItems.forEach(item => {
    if (!item.locked) {
        item[key] = Math.round((item[key] as number) * factor) as unknown as T[keyof T]
    }
  })
  ...
  if (diff !== 0) {
    ...
    if (firstUnlocked) firstUnlocked[key] = ((firstUnlocked[key] as number) + diff) as unknown as T[keyof T]
  }
