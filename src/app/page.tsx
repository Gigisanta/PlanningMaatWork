...
export default function Home() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // ⚡ Bolt: Process file uploads concurrently in small batches to prevent UI blocking
    // and memory spikes when reading many large PDFs simultaneously.
    const BATCH_SIZE = 5;
    ...
    for (let i = 0; i < filesArray.length; i += BATCH_SIZE) {
      ...
      if (i + BATCH_SIZE < filesArray.length) {
        // ⚡ Bolt: Yield to the main thread between batches to keep the UI responsive
