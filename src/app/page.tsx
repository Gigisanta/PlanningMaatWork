...
const defaultPlatformLinks: ConfigurableLink[] = [
  { name: 'IOL invertironline', url: 'https://www.invertironline.com/', icon: 'link', enabled: true },
  { name: 'Balanz', url: 'https://balanz.com/', icon: 'link', enabled: true },
  { name: 'Cocos', url: 'https://cocos.capital/', icon: 'link', enabled: false }
]

const defaultSocialLinks: ConfigurableLink[] = [
  { name: '@mi.inversion', url: 'https://instagram.com/', icon: 'instagram' },
              ...
              <Popover>
                ...
                <PopoverContent className="w-80 bg-white border-[#E8E6E0] shadow-xl rounded-xl" align="end">
                  <div className="space-y-3 p-1">
                    ...
                    <div><Label className="text-xs text-[var(--primary-light)]">Plataformas</Label>{platformLinks.map((link, i) => (<div key={i} className="flex gap-1 mt-1 items-center"><input type="checkbox" checked={link.enabled !== false} onChange={(e) => { const l = [...platformLinks]; l[i] = { ...l[i], enabled: e.target.checked }; setPlatformLinks(l) }} className="rounded text-[var(--primary)] mr-1" /><Input value={link.name} onChange={(e) => { const l = [...platformLinks]; l[i] = { ...l[i], name: e.target.value }; setPlatformLinks(l) }} className="h-7 text-xs w-20" /><Input value={link.url} onChange={(e) => { const l = [...platformLinks]; l[i] = { ...l[i], url: e.target.value }; setPlatformLinks(l) }} className="h-7 text-xs flex-1" /></div>))}</div>
