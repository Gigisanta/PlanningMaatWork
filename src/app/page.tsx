      ...
      <header className="bg-[var(--primary)] text-white md:py-3 py-2.5 px-4 shadow-sm flex-shrink-0 sticky top-0 z-50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
                                    ...
                                    {portfolioLibrary.length === 0 ? <p className="text-center py-8 text-[#7A8B80] text-xs">No hay carteras guardadas.</p> :
                                        portfolioLibrary.map(item => (
                                            <div key={item.id} className="p-3 bg-white rounded-xl flex items-center justify-between border border-[#E8E6E0] hover:border-[var(--accent)] transition-colors group">
                                                ...
                                                <Button variant="ghost" size="sm" aria-label="Eliminar de biblioteca" title="Eliminar de biblioteca" onClick={() => handleDeleteFromLibrary(item.id)} className="text-red-400 opacity-0 group-hover:opacity-100 h-8 w-8 p-0"><Trash2 className="w-3.5 h-3.5"/></Button>
                                            </div>
                                        ))
                                    }
                                </div>
            ...
            {isMobile ? (
              <Button variant="outline" size="icon" aria-label="Abrir configuración" title="Abrir configuración" onClick={() => setShowMobileSettings(true)} className="bg-white/10 border-white/15 text-white hover:bg-white/20 rounded-lg h-10 w-10"><Settings className="w-5 h-5" /></Button>
            ) : (
              <Popover>
                <PopoverTrigger asChild><Button variant="outline" size="icon" aria-label="Abrir configuración" title="Abrir configuración" className="bg-white/10 border-white/15 text-white hover:bg-white/20 rounded-lg h-9 w-9"><Settings className="w-4 h-4" /></Button></PopoverTrigger>
