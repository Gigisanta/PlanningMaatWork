...
export async function GET() {
  try {
    ...
    // with a single O(N) loop to calculate byStage and outreachStats.
    // Impact: Reduces time complexity from O(K*N) to O(N) and prevents unnecessary memory allocations.
    ...
    for (const l of outreachLogs) {
      if (l.status !== "pending") outreachStats.totalSent++;
      if (["delivered", "opened", "replied", "booked"].includes(l.status)) outreachStats.delivered++;
      if (["opened", "replied", "booked"].includes(l.status)) outreachStats.opened++;
      if (["replied", "booked"].includes(l.status)) outreachStats.replied++;
      if (l.status === "booked") outreachStats.booked++;
    }
