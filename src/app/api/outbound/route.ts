...
export async function GET() {
  try {
    ...
    // ⚡ Bolt: Consolidate O(N) filter chains into single reduce passes for performance
    const byStage = leads.reduce((acc, l) => {
      if (acc[l.stage] !== undefined) acc[l.stage]++;
      return acc;
    }, { new: 0, outreach: 0, qualified: 0, meeting: 0, proposal: 0, won: 0, lost: 0 } as Record<string, number>);

    const outreachStats = outreachLogs.reduce((acc, l) => {
