...
// ⚡ Bolt: Flatten keywords and move static arrays out of function scope
const FLATTENED_KEYWORDS = Object.values(CROSSSELL_KEYWORDS).flat();
...
function classifyLead(lead: Partial<Lead>): { primaryService: string; crossSellOpportunity: boolean; crossSellReason?: string } {
  ...
  const financialKeywordsFound = FLATTENED_KEYWORDS.filter(kw => text.includes(kw));
...
export async function GET() {
  try {
    ...
    // ⚡ Bolt: Consolidate multiple filter passes into a single reduce
    const stats = classified.reduce(
      (acc, l) => {
        if (l.primaryService === "maatwork") acc.maatwork++;
        if (l.primaryService === "cactuswealth") acc.cactuswealth++;
