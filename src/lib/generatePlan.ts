...
export function generatePlanHTML(data: PlanData): string {
  ...
  // ⚡ Bolt: Single O(N) pass to precompute all portfolio exposures
  // replacing 5 separate O(N) .filter().reduce() iterations inside the template literal
  const exposures = (data.instruments || []).reduce(
    (acc, i) => {
