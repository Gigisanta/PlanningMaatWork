...
## 2026-05-11 - Optimizing Redundant Array Iterations in Data Transformations
**Learning:** Consolidating multiple chained `.filter().length` and `.reduce()` calls into a single O(N) `for...of` loop significantly reduces time complexity and memory allocations in data transformation pipelines.
**Action:** When calculating multiple independent metrics from the same array, consolidate redundant `.filter()` chains into a single O(N) pass to accumulate all values into a single object or primitive variables.
