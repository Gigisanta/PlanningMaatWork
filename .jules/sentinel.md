...
## 2024-11-20 - [Insecure Randomness]
**Vulnerability:** `Math.random()` was used for generating IDs.
**Learning:** The application's API routes (`leads`, `outbound`, `classify`) used timestamp-based `Math.random()` generation pattern.
**Prevention:** Use standard UUID v4 (`randomUUID()` from `crypto`) for all entity identifiers.
